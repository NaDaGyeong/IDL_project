    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                #print(f"Train batch {i}: batch_x={batch_x.shape}, batch_y={batch_y.shape}, "
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                #print(f"Test batch {i}: batch_x={batch_x.shape}, batch_y={batch_y.shape}, "
          #f"batch_x_mark={batch_x_mark.shape}, batch_y_mark={batch_y_mark.shape}")
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        for _ in range(self.model.diffusion_config.testing.n_z_samples_depart):
                            for _ in range(self.model.diffusion_config.testing.n_z_samples_depart):
                                y_tile_seq = p_sample_loop(self.model, 
                                                           x_tile, 
                                                           x_mark_tile, 
                                                           y_0_hat_tile, 
