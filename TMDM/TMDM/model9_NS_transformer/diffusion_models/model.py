...
class ConditionalGuidedModel(nn.Module):
    def __init__(self, config, MTS_args):
        ...
        self.conv1d = nn.Conv1d(in_channels=7, out_channels=7, kernel_size=3, padding=1)
    
    def extract_local_context(self,y):
        #print(f"[DEBUG] extract_local_context input shape: {y.shape}")
        if y.shape[-1] != 7:
            y = y[..., :7]
        y = y.permute(0,2,1)
        #print(f"[DEBUG] Permuted input shape: {y.shape}")
        local_context = self.conv1d(y)
        #print(f"[DEBUG] Conv1D output shape: {local_context.shape}")
        return local_context.permute(0,2,1)
    ...
    def forward(self, x, y_t, y_0_hat, t):
        ...
        # return eps_pred
        local_context = self.extract_local_context(y_t)
        # Ensure local_context and y_0_hat have matching dimensions
        assert local_context.shape[-1] == y_0_hat.shape[-1], \
            f"Dimension mismatch: local_context={local_context.shape}, y_0_hat={y_0_hat.shape}"
        combined_context = torch.cat((y_0_hat, local_context), dim=-1)

        eps_pred = F.softplus(self.lin1(combined_context, t))
        eps_pred = F.softplus(self.lin2(eps_pred, t))
        eps_pred = F.softplus(self.lin3(eps_pred, t))
        eps_pred = self.lin4(eps_pred)
        return eps_pred
