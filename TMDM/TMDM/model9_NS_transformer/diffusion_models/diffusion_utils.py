...
def p_sample(model, x, x_mark, y, y_0_hat, t, alphas, one_minus_alphas_bar_sqrt):
    """
    ...
    local_context = model.extract_local_context(y_adjusted)
    combined_context = torch.cat((local_context, y_0_hat), dim=-1)

    eps_theta = model(x, x_mark, 0, y, y_0_hat, t).to(device).detach()
    #eps_theta = model(x, x_mark, combined_context, y, t).to(device).detach()
    #eps_theta = model.diffussion_model(x, combined_context, y, t).to(device).detach()
    ...
    y_t_minus_1 = (sqrt_alpha_bar_t * y) - eps_theta * sqrt_one_minus_alpha_bar_t + z * sqrt_one_minus_alpha_bar_t
    # posterior variance
...
def p_sample_t_1to0(model, x, x_mark, y_t, y_0_hat, one_minus_alphas_bar_sqrt):
    device = next(model.parameters()).device
    t = torch.tensor([0]).to(device)  # corresponding to timestep 1 (i.e., t=1 in diffusion models)

    local_context = model.extract_local_context(y_t)
    combined_context = torch.cat((local_context, y_0_hat), dim=-1)


    sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, y_t)
    sqrt_alpha_bar_t = (1 - sqrt_one_minus_alpha_bar_t.square()).sqrt()
    eps_theta = model(x, x_mark, 0, y_t, y_0_hat, t).to(device).detach()
    #eps_theta = model(x, x_mark, combined_context, y, t).to(device).detach()
    #eps_theta = model(x, combined_context, y, t).to(device).detach()
    #eps_theta = model(x, y, y_0_hat, t).to(device).detach()
    #eps_theta = model(x, y_t, y_0_hat, t).to(device).detach()
    #eps_theta = model(x, x_mark, y_t, y_0_hat, t).to(device).detach()
    #eps_theta = model(x, y_t, y_0_hat, t).to(device).detach()
    # y_0 reparameterization
    #y_0_reparam = 1 / sqrt_alpha_bar_t * (
    #        y - (1 - sqrt_alpha_bar_t) * y_T_mean - eps_theta * sqrt_one_minus_alpha_bar_t)
    y_0_reparam = (1 / sqrt_alpha_bar_t) * (y_t - eps_theta * sqrt_one_minus_alpha_bar_t)
    y_t_m_1 = y_0_reparam.to(device)
    return y_t_m_1


def p_sample_loop(model, x, x_mark, y_0_hat, n_steps, alphas, one_minus_alphas_bar_sqrt):
    device = next(model.parameters()).device
    #z = torch.randn_like(y_T_mean).to(device)
    z = torch.randn_like(y_0_hat).to(device)
    ...
    for t in reversed(range(1, n_steps)):  # t from T to 2
        ...
        cur_y = p_sample(model, x, x_mark, cur_y, y_0_hat, t, alphas, one_minus_alphas_bar_sqrt)
    ...
    y_0 = p_sample_t_1to0(model, x, x_mark, y_p_seq[-1], y_0_hat, one_minus_alphas_bar_sqrt)
    y_p_seq.append(y_0)
    return y_p_seq
